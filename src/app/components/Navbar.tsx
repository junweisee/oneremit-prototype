...
import imgFlagEN  from 'figma:asset/9875e3b3d86e7e562cb0bc2c432eba5885fc02be.png';
import imgFlagKR  from 'figma:asset/da19d3b20c36de9cd3c0b60eb2dd732efebf6d02.png';
...
export function Navbar({ lang, setLang, t }: NavbarProps) {
  ...
  useEffect(() => {
    function handleClickOutside(e: MouseEvent) {
      ...
      if (!desktopLangRef.current?.contains(target) && !mobileLangRef.current?.contains(target)) {
        setLangOpen(false);
      }
  ...
  return (
    ...
    >
      <div className="max-w-[1440px] mx-auto flex items-center justify-between px-4 md:px-16 py-3 md:py-2">
        ...
        {/* Logo */}
        <Link to="/" className="h-10 w-[100px] lg:h-[50px] lg:w-[131px] relative shrink-0">
        ...
        <div className="hidden lg:flex items-center gap-10">
          ...
          {/* Send money */}
          <Link to="/" className="text-[#3f4e50] text-[15px] font-medium cursor-pointer">
            {t.nav.sendMoney}
          </Link>
          ...
          <div className="relative" ref={desktopHtuRef}>
            ...
            >
              {t.nav.howToUse}
              <ChevronDown size={14} className={`transition-transform duration-200 ${howToUseOpen ? 'rotate-180' : ''}`} color={isHtu ? '#3a9944' : '#8CA2A4'} />
            </button>
            <AnimatePresence>
              {howToUseOpen && (
                <motion.div
        ...
        <div className="hidden lg:flex items-center gap-10">
          ...
          <div className="relative" ref={desktopLangRef}>
            ...
            >
              <div className="relative size-6 overflow-hidden rounded-sm">
                <img src={FLAG_MAP[lang]} alt={lang} className="absolute inset-0 w-full h-full object-cover" />
              </div>
              <span className="text-[#3f4e50] text-[14px] font-medium">{currentLang.label}</span>
              <svg width="10" height="6" viewBox="0 0 10 6" fill="none" className={`transition-transform duration-200 ${langOpen ? 'rotate-180' : ''}`}>
                <path d="M1 1L5 5L9 1" stroke="#8CA2A4" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
              </svg>
            </button>
            <AnimatePresence>
              {langOpen && (
                  ...
                  {LANGUAGES.map(l => (
                    ...
                    >
                      <div className="relative size-5 overflow-hidden rounded-sm shrink-0">
                        <img src={FLAG_MAP[l.code]} alt={l.label} className="absolute inset-0 w-full h-full object-cover" />
                      </div>
                      <span className="text-[#3f4e50] text-[13px] font-medium">{l.label}</span>
                      <span className="text-[#8CA2A4] text-[12px]">{l.name}</span>
                    </button>
                  ))}
                </motion.div>
              )}
          ...
          <div className="flex items-center gap-6">
            <button className="text-[#3a9944] text-[15px] font-medium cursor-pointer">{t.nav.login}</button>
        ...
        <div className="flex lg:hidden items-center gap-3">
          ...
          <div className="relative" ref={mobileLangRef}>
            ...
            >
              ...
              <span className="text-[#3f4e50] text-[12px] font-medium">{currentLang.label}</span>
            </button>
            <AnimatePresence>
              {langOpen && (
                <motion.div
                ...
                >
                  {LANGUAGES.map(l => (
                    ...
                    >
                      ...
                      <span className="text-[#3f4e50] text-[12px]">{l.label}</span>
                    </button>
                  ))}
                </motion.div>
              )}
          ...
          {/* Hamburger */}
          <button className="cursor-pointer" onClick={() => setMobileOpen(v => !v)}>
            {mobileOpen ? <X size={22} color="#3f4e50" /> : <Menu size={22} color="#3f4e50" />}
          </button>
        </div>
      </div>
      ...
      <AnimatePresence>
        {mobileOpen && (
          ...
          >
            <Link to="/"      className="text-[#3f4e50] text-[15px] font-medium py-2 block">{t.nav.sendMoney}</Link>
            ...
            >
              {t.nav.howToUse}
              <ChevronDown size={14} className={`transition-transform duration-200 ${mobileHtu ? 'rotate-180' : ''}`} color="#8CA2A4" />
            </button>
            <AnimatePresence>
              {mobileHtu && (
                <motion.div
                ...
                >
                  {HOW_TO_USE_LINKS.map(link => (
                    <Link key={link.to} to={link.to} className="text-[#647576] text-[14px] py-1.5 block">
                      {link.label}
                    </Link>
                  ))}
                </motion.div>
            ...
            <Link to="/about" className="text-[#3f4e50] text-[15px] font-medium py-2 block">{t.nav.aboutUs}</Link>
            <Link to="/help"  className="text-[#3f4e50] text-[15px] font-medium py-2 block">{t.nav.help}</Link>
